...
const ManutencaoLivros = () => {
    ...
    const filtrarLista = async (campos) => {
        try {
            ...
            lista.data.length
                ? setLivros(lista.data)
                : alert("Nã há livros com a palavra-chave pesquisada...");
            ...
            <table className="table table-striped mt-3">
                ...
                <tbody>
                    {livros.map((livros, index) => (
                        <ItemLista key={livros.id} id={livros.id} titulo={livros.titulo}
                            autor={livros.autor} ano={livros.ano} preco={livros.preco}
                            foto={livros.foto}
